...
int serverTcp() {
	...
	saddr.sin_port = htons(6969); // Ö¸¶¨¶Ë¿ÚºÅ
	saddr.sin_addr.s_addr = INADDR_ANY; // ·þÎñÆ÷¼àÌýËùÓÐÍøÂç½Ó¿Ú
	...
	// ¿ªÊ¼¼àÌýÁ¬½ÓÇëÇó
	ret = listen(server_fd, 66); // ÉèÖÃ×î´óÍ¬Ê±Á¬½ÓÊýÎª 66
	if (ret == -1) {
		perror("listen");
		close(server_fd);
		return -1;
    ...
    //´´½¨epollÊµÀý
	int epoll_fd = epoll_create(1);//²ÎÊý¶ÔÆäÃ»ÓÐÈÎºÎÓ°Ïì£¬µ«ÊÇÒªÇó´óÓÚ0
	if (epoll_fd == -1)
	{
		close(server_fd);
	...
	ev[count_event].events = EPOLLIN;//¼ì²â¶ÁÊÂ¼þ
...
void accept(void* arg)
{
	...
	socklen_t len = sizeof(addr); // ³õÊ¼»¯³¤¶È
	int size = sizeof(events) / sizeof(events[0]);
	while (true)
	{
		int num=epoll_wait(acceptinfo->epfd, events, size, -1);//-1´ú±íÓÐÎÄ¼þ±íÊ¾·û¾ÍÐ÷£¬·µ»ØÖµnumÊÇ·µ»ØµÄ¾ÍÐ÷µÄÎÄ¼þ±íÊ¾·ûµÄ×Ü¸öÊý
		for (int i = 0; i < num; i++)
		{
